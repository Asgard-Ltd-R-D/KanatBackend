...
public static class OnVifPacketFilter
{
    public static async Task<IEnumerable<PlainDataDto>> FilterAsync(IEnumerable<OnVIFPacketEntity> items, IReadOnlyDictionary<string, object>? filters)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            var filteredItems = items.Where(item => 
            {
                ...
                {
                    var property = typeof(OnVIFPacketEntity).GetProperty(filter.Key, BindingFlags.Public | BindingFlags.Instance | BindingFlags.IgnoreCase);
